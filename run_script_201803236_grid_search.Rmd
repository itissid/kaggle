...
grid.scores.topk =  Reduce(rbind,
                           Map(function(m) c(
                                             ...
                                             validation_rmse=h2o.rmse(h2o.performance(m, valid=TRUE)),
                                             model_id=m@model_id),
...
xval.scores.topk = Reduce(rbind,
    Map(function(i) {
            m = cvgbms[[topk.ids[i]]]
            if(is.null(m)) {
                # Due to failures some of the models can be NULL
                flog.warn(paste0("CV slot for model #",i, ", id: ", topk.ids[[i]]," was null."))
                return(rep(NA,4))
            } else {
                return(data.frame(
                          training_cv_rmse=h2o.rmse(h2o.performance(m, train=TRUE)),
                          five_fold_cv_rmse=h2o.rmse(h2o.performance(m, xval=TRUE)),
                          five_fold_cv_rmse_sd=as.numeric(m@model$cross_validation_metrics_summary['rmse', ]$sd),
                          validation_cv_rmse=h2o.rmse(h2o.performance(m, valid=TRUE)),
                          model_id=topk.ids[[i]]))
            }},
         1:20))
...
combined.scores = dplyr::inner_join(grid.scores.topk, xval.scores.topk, by=c("model_id")) %>% data.frame

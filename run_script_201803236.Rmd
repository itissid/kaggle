...
list[tr.baseline, pr.baseline, vtreatIdentityFn, tplan.NULL] =  prepareDataWrapper.h2o.gbm.baseline(dates.to.numeric=T, keep.dates=T)
...
Lets create a grid that only varies the learning rate only. We choose a large number of trees. But becasue we have early stopping we should be ok.
...
These values of depth(10-14) and learning_rate(0.01,0.02) will be the parameters that shall be used in the grid search.

In the next we do some predictions using the best model from just these values.

```{r}
#model
#gbm.bestfit = h2o.getModel(model_id)
```
